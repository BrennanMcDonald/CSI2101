...
class LinkList {
	...
	LinkList( int sz ) {
		if ( sz <= 0 ) {
			...
			llist = new GNode<Integer>(0, null);
			GNode<Integer> current = llist; // temp node for loop
			// add further nodes
			for ( int i = 1; i < sz; ++i ) {
				GNode<Integer> node2Add = new GNode<Integer>(i, null);
	...
	/**
	 * Print all the elements of the list assuming that t22hey are Strings
	 */
	public void print() {
		/* Print the list */
		GNode<Integer> current = llist; // point to the first node
		while (current != null) {
			System.out.print((Integer)current.getElement() + " ");
	...
	public void deleteLast() {
		...
		GNode<Integer> prev = llist;
		GNode<Integer> current = prev.getNext();
